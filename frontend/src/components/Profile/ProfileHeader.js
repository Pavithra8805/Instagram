...
const ProfileHeader = ({ username, postCount, user, updateNewPost }) => {
  ...
  const handleUploadPic = async (event) => {
    ...
    if (image) {
      ...
      const urlInfo = supabase.storage.from('images').getPublicUrl(data.path);
  ...
  return (
    <div className="flex items-center p-4">
      ...
      {showProfilePicModal && (
        <div className="flex items-center justify-center h-full fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
